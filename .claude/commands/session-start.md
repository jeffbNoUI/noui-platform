...
```bash
git fetch origin
git merge --ff-only origin/main 2>/dev/null || echo "Already up to date or not on main branch"
```

## Step 3: Show Project State

Read `BUILD_HISTORY.md` — understand current state, last changes, any open issues.
Check for any `.claude/prompts/` files relevant to the current task.

Also check for unresolved error reports (skip silently if the Issues service is not reachable):
...
Quick Reference:
  ...
  /simplify        After committing — iterative cleanup (up to 3 cycles)
