// Employer SCP (Service Credit Purchase) service — cost factor lookup,
// purchase requests, exclusion flag enforcement, payment tracking.
package main
...
import (
	...
	"github.com/noui/platform/employer-scp/api"
	"github.com/noui/platform/employer-scp/db"
...
func main() {
	startedAt := time.Now()
	logger := logging.Setup("employer-scp", nil)
	slog.SetDefault(logger)
	slog.Info("starting employer-scp service v0.1.0")
	...
	mux.HandleFunc("GET /health/detail", healthutil.NewDetailHandler("employer-scp", "0.1.0", startedAt, database, counters))
	mux.HandleFunc("GET /ready", healthutil.NewReadyHandler("employer-scp", database))
	...
	go func() {
		slog.Info("employer-scp service listening", "port", port)
	...
	<-done
	slog.Info("shutting down employer-scp service...")
	...
	slog.Info("employer-scp service stopped")
}
