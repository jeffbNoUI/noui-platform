// Employer Enrollment service — new member submissions, duplicate detection,
// PERAChoice election tracking for employer-initiated and member-initiated enrollments.
package main
...
import (
	...
	"github.com/noui/platform/employer-enrollment/api"
	"github.com/noui/platform/employer-enrollment/db"
...
func main() {
	startedAt := time.Now()
	logger := logging.Setup("employer-enrollment", nil)
	slog.SetDefault(logger)
	slog.Info("starting employer-enrollment service v0.1.0")
	...
	mux.HandleFunc("GET /health/detail", healthutil.NewDetailHandler("employer-enrollment", "0.1.0", startedAt, database, counters))
	mux.HandleFunc("GET /ready", healthutil.NewReadyHandler("employer-enrollment", database))
	...
	go func() {
		slog.Info("employer-enrollment service listening", "port", port)
	...
	<-done
	slog.Info("shutting down employer-enrollment service...")
	...
	slog.Info("employer-enrollment service stopped")
}
