...
func TestCreateEngagement(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO migration.engagement").
		WithArgs("tenant-1", "LegacyPAS", nil, "standard").
		WillReturnRows(sqlmock.NewRows(engagementCols).AddRow(
			"eng-001", "tenant-1", "LegacyPAS", "1.0",
			"DISCOVERY", nil, nil, nil, "standard", now, now,
		))

	e, err := CreateEngagement(db, "tenant-1", "LegacyPAS", nil, "")
	if err != nil {
...
func TestListEngagements(t *testing.T) {
	...
	rows := sqlmock.NewRows(engagementCols).
		AddRow("eng-002", "tenant-1", "SystemB", "1.0", "PROFILING", nil, nil, nil, "standard", now, now).
		AddRow("eng-001", "tenant-1", "SystemA", "1.0", "MAPPING", nil, nil, nil, "standard", now.Add(-time.Hour), now)
