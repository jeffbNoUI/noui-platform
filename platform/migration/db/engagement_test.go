...
func TestListEngagements(t *testing.T) {
	...
	rows := sqlmock.NewRows(engagementCols).
		AddRow("eng-002", "tenant-1", "SystemB", "1.0", "PROFILING", nil, nil, nil, now, now).
		AddRow("eng-001", "tenant-1", "SystemA", "1.0", "MAPPING", nil, nil, nil, now.Add(-time.Hour), now)
