// Issue Management service — tracks issues, defects, and enhancements for the NoUI Services Hub.
package main
...
import (
	...
	"github.com/noui/platform/issues/api"
	"github.com/noui/platform/issues/db"
...
func main() {
	startedAt := time.Now()
	logger := logging.Setup("issues", nil)
	slog.SetDefault(logger)
	slog.Info("starting Issue Management service v0.1.0")
	...
	mux.HandleFunc("GET /health/detail", healthutil.NewDetailHandler("issues", "0.1.0", startedAt, database, counters))
	mux.HandleFunc("GET /ready", healthutil.NewReadyHandler("issues", database))
	...
	go func() {
		slog.Info("Issue Management service listening", "port", port)
	...
	<-done
	slog.Info("shutting down Issue Management service...")
	...
	slog.Info("Issue Management service stopped")
}
