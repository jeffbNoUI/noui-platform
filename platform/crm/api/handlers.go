...
func (h *Handler) GetContact(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "NOT_FOUND", "Contact not found")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "DB_ERROR", err.Error())
		return
...
func (h *Handler) GetContactByLegacyID(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "NOT_FOUND", "Contact not found for legacy ID")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "DB_ERROR", err.Error())
		return
...
func (h *Handler) GetConversation(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "NOT_FOUND", "Conversation not found")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "DB_ERROR", err.Error())
		return
...
func (h *Handler) GetInteraction(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "NOT_FOUND", "Interaction not found")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "DB_ERROR", err.Error())
		return
...
func (h *Handler) CreateNote(w http.ResponseWriter, r *http.Request) {
	...
	if req.AIConfidence != nil {
		...
		if err != nil {
			apiresponse.WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "aiConfidence must be a decimal number")
			return
		}
		note.AIConfidence = &v
	}
...
func (h *Handler) UpdateCommitment(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "NOT_FOUND", "Commitment not found")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "DB_ERROR", err.Error())
		return
...
func (h *Handler) UpdateOutreach(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "NOT_FOUND", "Outreach not found")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "DB_ERROR", err.Error())
		return
...
func (h *Handler) GetOrganization(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "NOT_FOUND", "Organization not found")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "DB_ERROR", err.Error())
		return
