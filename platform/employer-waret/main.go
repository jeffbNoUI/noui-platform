// Employer WARET (Working After Retirement) service — designations, tracking,
// penalties with penny-accurate arithmetic, PERACare conflict detection.
package main
...
import (
	...
	"github.com/noui/platform/employer-waret/api"
	"github.com/noui/platform/employer-waret/db"
...
func main() {
	startedAt := time.Now()
	logger := logging.Setup("employer-waret", nil)
	slog.SetDefault(logger)
	slog.Info("starting employer-waret service v0.1.0")
	...
	mux.HandleFunc("GET /health/detail", healthutil.NewDetailHandler("employer-waret", "0.1.0", startedAt, database, counters))
	mux.HandleFunc("GET /ready", healthutil.NewReadyHandler("employer-waret", database))
	...
	go func() {
		slog.Info("employer-waret service listening", "port", port)
	...
	<-done
	slog.Info("shutting down employer-waret service...")
	...
	slog.Info("employer-waret service stopped")
}
