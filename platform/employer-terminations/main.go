// Employer Terminations service — termination certifications, certification holds,
// refund applications with eligibility checks and penny-accurate refund calculations.
package main
...
import (
	...
	"github.com/noui/platform/employer-terminations/api"
	"github.com/noui/platform/employer-terminations/db"
...
func main() {
	startedAt := time.Now()
	logger := logging.Setup("employer-terminations", nil)
	slog.SetDefault(logger)
	slog.Info("starting employer-terminations service v0.1.0")
	...
	mux.HandleFunc("GET /health/detail", healthutil.NewDetailHandler("employer-terminations", "0.1.0", startedAt, database, counters))
	mux.HandleFunc("GET /ready", healthutil.NewReadyHandler("employer-terminations", database))
	...
	go func() {
		slog.Info("employer-terminations service listening", "port", port)
	...
	<-done
	slog.Info("shutting down employer-terminations service...")
	...
	slog.Info("employer-terminations service stopped")
}
