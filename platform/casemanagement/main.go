...
func main() {
	startedAt := time.Now()
	logger := logging.Setup("casemanagement", nil)
	slog.SetDefault(logger)
	slog.Info("starting Case Management service v0.1.0")
	...
	mux.HandleFunc("GET /health/detail", healthutil.NewDetailHandler("casemanagement", "0.1.0", startedAt, database, counters))
	mux.HandleFunc("GET /ready", healthutil.NewReadyHandler("casemanagement", database))
	...
	go func() {
		slog.Info("Case Management service listening", "port", port)
