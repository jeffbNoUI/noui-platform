...
func (h *Handler) GetCase(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "casemanagement", "NOT_FOUND", "Case not found")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "casemanagement", "DB_ERROR", err.Error())
		return
...
func (h *Handler) AdvanceStage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		apiresponse.WriteError(w, http.StatusBadRequest, "casemanagement", "ADVANCE_ERROR", err.Error())
		return
	}
...
func (h *Handler) DeleteNote(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		apiresponse.WriteError(w, http.StatusBadRequest, "casemanagement", "INVALID_REQUEST", "invalid noteId")
		return
	}

	if err := h.store.DeleteNote(r.Context(), caseID, noteID); err != nil {
		if err == cmdb.ErrNotFound {
			apiresponse.WriteError(w, http.StatusNotFound, "casemanagement", "NOT_FOUND", "Note not found")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "casemanagement", "DB_ERROR", err.Error())
		return
...
func (h *Handler) DeleteDocument(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		apiresponse.WriteError(w, http.StatusBadRequest, "casemanagement", "INVALID_REQUEST", "invalid docId")
		return
	}

	if err := h.store.DeleteDocument(r.Context(), caseID, docID); err != nil {
		if err == cmdb.ErrNotFound {
			apiresponse.WriteError(w, http.StatusNotFound, "casemanagement", "NOT_FOUND", "Document not found")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "casemanagement", "DB_ERROR", err.Error())
		return
