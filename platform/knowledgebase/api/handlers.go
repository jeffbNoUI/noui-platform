...
func (h *Handler) GetArticle(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "NOT_FOUND", "Article not found")
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "DB_ERROR", err.Error())
		return
...
func (h *Handler) GetStageHelp(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "NOT_FOUND", "No help found for stage: "+stageID)
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "DB_ERROR", err.Error())
		return
...
func (h *Handler) SearchArticles(w http.ResponseWriter, r *http.Request) {
	...
	query := r.URL.Query().Get("q")
	if query == "" {
		apiresponse.WriteError(w, http.StatusBadRequest, "INVALID_REQUEST", "Search query 'q' is required")
		return
	}
...
func (h *Handler) GetRule(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			apiresponse.WriteError(w, http.StatusNotFound, "NOT_FOUND", "Rule not found: "+ruleID)
			return
		}
		apiresponse.WriteError(w, http.StatusInternalServerError, "DB_ERROR", err.Error())
		return
