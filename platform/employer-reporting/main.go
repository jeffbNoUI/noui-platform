// Employer Reporting service — contribution file validation, exception management,
// payment setup, and late interest tracking for employer payroll submissions.
package main
...
import (
	...
	"github.com/noui/platform/employer-reporting/api"
	"github.com/noui/platform/employer-reporting/db"
...
func main() {
	startedAt := time.Now()
	logger := logging.Setup("employer-reporting", nil)
	slog.SetDefault(logger)
	slog.Info("starting employer-reporting service v0.1.0")
	...
	mux.HandleFunc("GET /health/detail", healthutil.NewDetailHandler("employer-reporting", "0.1.0", startedAt, database, counters))
	mux.HandleFunc("GET /ready", healthutil.NewReadyHandler("employer-reporting", database))
	...
	go func() {
		slog.Info("employer-reporting service listening", "port", port)
	...
	<-done
	slog.Info("shutting down employer-reporting service...")
	...
	slog.Info("employer-reporting service stopped")
}
