...
| 2026-03-25 | 3 independent reviewers (Round 3: coherence, TDD readiness, ops) | git rebase -i in headless (FAIL), Night 1 parallelism wrong (FAIL), recon table missing columns (FAIL), risk DEFERRED not in schema (FAIL), N=0 squash guard (FAIL), SESSION_DISPATCH not discoverable (FAIL). | Fixed all. Added /execute-contract command, worktree fallback, TDD failure modes, autonomous overrides, one-contract-per-run batch model. |
| 2026-03-25 | 3 persona reviewers (Round 4: T1 Auditor, T2 Analyst, T3 SRE) | T1: M00 missing 8 canonical tables (WARN), test data hygiene missing (WARN), label not access-controlled (FAIL). T2: resolved_by VARCHAR inconsistent (WARN), risk_handlers rejects DEFERRED (WARN), cross-source mismatch untested (WARN). T3: N=0 guard missing (FAIL), --base master residual (FAIL), shell metachar risk (WARN). | Fixed: M00 expanded to 22 tables, M01 VARCHAR(200) + risk_handlers + AC-9, N=0 guard, --base main, test data hygiene in rubric, M02a relaxed to M00-only dependency. |
