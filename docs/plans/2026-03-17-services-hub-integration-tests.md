# Services Hub Integration Tests — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a bash/curl/jq E2E test script that verifies all 7 Services Hub tabs receive valid data from real backend services running in Docker Compose.

**Architecture:** Single script `tests/e2e/services_hub_e2e.sh` matching the established `correspondence_e2e.sh` pattern. Hits endpoints through nginx proxy on localhost:3000. Uses seeded data for read tests and creates/mutates records for CRUD tests.

**Tech Stack:** Bash, curl, jq. No new dependencies.

---

### Task 1: Script scaffold with shared helpers

**Files:**
- Create: `tests/e2e/services_hub_e2e.sh`

**Step 1: Create the script with header, helpers, and --wait flag**
...
**Step 2: Verify the scaffold is syntactically valid**

Run: `bash -n tests/e2e/services_hub_e2e.sh`
Expected: No output (no syntax errors)

**Step 3: Commit**

```bash
git add tests/e2e/services_hub_e2e.sh
git commit -m "[tests/e2e] Add services hub integration test scaffold"
```

---

### Task 2: Section 1 — Health Tab

**Files:**
- Modify: `tests/e2e/services_hub_e2e.sh`

**Step 1: Add Health Tab section**

Append after the header echo:
...
```bash
git add tests/e2e/services_hub_e2e.sh
git commit -m "[tests/e2e] Add Health tab integration tests"
```

---

### Task 3: Section 2 — Data Quality Tab

**Files:**
- Modify: `tests/e2e/services_hub_e2e.sh`

**Step 1: Add Data Quality section**
...
```bash
git add tests/e2e/services_hub_e2e.sh
git commit -m "[tests/e2e] Add Data Quality tab integration tests"
```

---

### Task 4: Section 3 — Audit Trail Tab
...
```bash
bash -n tests/e2e/services_hub_e2e.sh
git add tests/e2e/services_hub_e2e.sh
git commit -m "[tests/e2e] Add Audit Trail tab integration tests"
```

---

### Task 5: Section 4 — Metrics Tab
...
bash -n tests/e2e/services_hub_e2e.sh
git add tests/e2e/services_hub_e2e.sh
git commit -m "[tests/e2e] Add Metrics tab integration tests"
```

---

### Task 6: Section 5 — Security Tab (CRUD)

**Files:**
- Modify: `tests/e2e/services_hub_e2e.sh`

**Step 1: Add Security section with POST + GET round-trip**
...
bash -n tests/e2e/services_hub_e2e.sh
git add tests/e2e/services_hub_e2e.sh
git commit -m "[tests/e2e] Add Security tab integration tests with CRUD"
```

---

### Task 7: Section 6 — Issues Tab (full CRUD lifecycle)

**Files:**
- Modify: `tests/e2e/services_hub_e2e.sh`

**Step 1: Add Issues section with Create → Read → Update → Filter**
...
bash -n tests/e2e/services_hub_e2e.sh
git add tests/e2e/services_hub_e2e.sh
git commit -m "[tests/e2e] Add Issues tab integration tests with full CRUD"
```

---

### Task 8: Section 7 — Config Tab + Summary

**Files:**
- Modify: `tests/e2e/services_hub_e2e.sh`

**Step 1: Add Config section and summary footer**
...
```bash
git add tests/e2e/services_hub_e2e.sh
git commit -m "[tests/e2e] Add Config tab tests + summary — complete script"
```

---

### Task 9: Final verification — run against Docker stack

**Step 1: Boot Docker stack if not running**
...
If any section fails, diagnose by:
1. Check the HTTP code — 000 means service unreachable, 404 means wrong path
2. Check the response body — pipe to `jq .` for formatting
3. Check Docker logs — `docker compose logs <service> --tail 20`

**Step 4: Final commit if fixes were needed**

```bash
git add tests/e2e/services_hub_e2e.sh
git commit -m "[tests/e2e] Fix services hub integration test failures"
```

---

### Task 10: Update BUILD_HISTORY.md
...
Add at the top of BUILD_HISTORY.md (below the title):
...
**What was built:**
- `tests/e2e/services_hub_e2e.sh` — 30+ curl/jq assertions across 7 sections
- Health: aggregate status, per-service health, service count
- Data Quality: score, checks list, issues list, status update round-trip
...
- Issues: full CRUD lifecycle (create → read → update → filter)
- Config: rules list, field presence

**Test Results:**
- N assertions passed, 0 failed (update after run)
...
git commit -m "[docs] Update BUILD_HISTORY.md with integration tests session"
```
