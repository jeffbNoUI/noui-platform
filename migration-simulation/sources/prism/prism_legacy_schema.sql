...
CREATE TABLE PRISM_MEMBER (
    ...
    PLAN_CD         VARCHAR(6)   NOT NULL,   -- References PRISM_MISC_CODES TYPE_CD='PLAN'. No FK declared.
    ...
    LST_UPD_DT      DATETIME     NULL,       -- Last update timestamp
...
CREATE TABLE PRISM_EMP_SPELL (
    ...
    SPELL_TYP       CHAR(2)      NULL,       -- 'FT'=full-time,'PT'=part-time,'TMP'=temporary,'LV'=leave
    FTE_PCT         DECIMAL(5,2) NULL,       -- Full-time equivalent percentage. NULL for older records.
...
CREATE TABLE PRISM_JOB_HIST (
    ...
    ANNUAL_SAL      DECIMAL(12,2) NULL,      -- Annual salary at time of action. NULL for some part-time.
    HRLY_RATE       DECIMAL(8,4)  NULL,      -- Hourly rate. Populated if EXEMPT_FLG='N'. Conflicts with ANNUAL_SAL.
    EXEMPT_FLG      CHAR(1)      NULL,       -- 'Y'=exempt (salaried), 'N'=non-exempt (hourly). NULL for old records.
    STD_HRS_WK      DECIMAL(5,2) NULL,       -- Standard hours per week. 40.0 for most FT.
...
CREATE TABLE PRISM_SAL_ANNUAL (
    ...
    GROSS_EARN      DECIMAL(12,2) NOT NULL,  -- Total gross earnings for the year
    PENSION_EARN    DECIMAL(12,2) NULL,      -- Pension-eligible earnings. NULL = assume = GROSS_EARN.
    CONTRIB_AMT     DECIMAL(10,2) NULL,      -- Employee contributions for the year (denormalized here AND in CONTRIB_LEGACY)
...
CREATE TABLE PRISM_SAL_PERIOD (
    ...
    GROSS_PAY       DECIMAL(12,2) NOT NULL,  -- Gross pay for period
    PENSION_PAY     DECIMAL(12,2) NULL,      -- Pension-eligible pay. NULL = use GROSS_PAY.
    OVERTIME_PAY    DECIMAL(10,2) NULL,
    BONUS_PAY       DECIMAL(10,2) NULL,      -- Included in GROSS_PAY, broken out here
    HOURS_WORKED    DECIMAL(6,2) NULL,       -- NULL for exempt/salaried employees
    STD_HRS         DECIMAL(6,2) NULL,
...
CREATE TABLE PRISM_SVC_CREDIT (
    ...
    SVC_CR_BAL      DECIMAL(8,4) NOT NULL,   -- Cumulative credited service (years). Running balance.
    SVC_CR_YTD      DECIMAL(8,4) NULL,       -- Year-to-date accrual. Resets annually.
    SVC_TYP_CD      CHAR(4)      NULL,       -- 'ACTV'=active service,'LMIL'=military leave,'PRCH'=purchased,'TRNFR'=transfer
    PRCH_AMT        DECIMAL(10,2) NULL,      -- If SVC_TYP_CD='PRCH': amount paid for service purchase
...
CREATE TABLE PRISM_CONTRIB_LEGACY (
    ...
    EE_CONTRIB      DECIMAL(10,2) NOT NULL,  -- Employee contribution for year
    ER_CONTRIB      DECIMAL(10,2) NULL,      -- Employer contribution. NULL for most records (not tracked pre-1998).
    CONTRIB_RATE    DECIMAL(5,4) NULL,       -- Contribution rate (e.g. 0.0850 = 8.5%). NULL for many records.
    INT_CREDITED    DECIMAL(10,2) NULL,      -- Interest credited on account. NULL for DB plans (not tracked).
...
CREATE TABLE PRISM_CONTRIB_HIST (
    ...
    EE_CONTRIB_AMT  DECIMAL(10,2) NOT NULL,  -- Employee contribution
    ER_CONTRIB_AMT  DECIMAL(10,2) NULL,      -- Employer matching/contribution
    CONTRIB_RATE    DECIMAL(5,4) NULL,
    PENSION_SAL_BS  DECIMAL(12,2) NULL,      -- Pensionable salary basis for this period
    INT_AMT         DECIMAL(8,2) NULL,       -- Interest for DC plans. NULL for pure DB plans.
...
CREATE TABLE PRISM_BENEFIT_CALC (
    ...
    MBR_NBR         INT          NOT NULL,   -- Joins to PRISM_MEMBER.MBR_NBR
    CALC_DT         DATETIME     NOT NULL,   -- When calculation was run
    ...
    YOS_USED        DECIMAL(8,4) NULL,       -- Years of service used
    FAS_USED        DECIMAL(12,2) NULL,      -- Final average salary used
    FAS_PERIOD_MO   SMALLINT     NULL,       -- FAS averaging period in months (36 or 60)
    AGE_AT_CALC     DECIMAL(6,3) NULL,       -- Member age at as-of date
    RET_TYPE_CD     CHAR(3)      NULL,       -- 'NRM'=normal,'ERY'=early,'DSB'=disability,'SRV'=survivor
    -- Outputs
    GROSS_BENEFIT   DECIMAL(12,2) NULL,      -- Monthly gross benefit before deductions
    CALC_RESULT     DECIMAL(12,2) NULL,      -- Final monthly benefit amount (after all adjustments)
    ERY_REDUCTION   DECIMAL(6,4) NULL,       -- Early retirement reduction factor applied
    OPT_FORM_CD     VARCHAR(6)   NULL,       -- Payment form selected (life-only, J&S 50%, etc.)
    OPT_FORM_FACTOR DECIMAL(8,6) NULL,       -- Actuarial factor for payment form
    -- Audit
...
CREATE TABLE PRISM_PMT_SCHEDULE (
    ...
    MONTHLY_AMT     DECIMAL(12,2) NOT NULL,  -- Base monthly amount (before COLA etc.)
    ...
    TAX_ELECT_FED   DECIMAL(5,2) NULL,       -- Federal withholding amount or rate
    TAX_ELECT_STE   DECIMAL(5,2) NULL,       -- State withholding
...
CREATE TABLE PRISM_PMT_HIST (
    ...
    GROSS_AMT       DECIMAL(12,2) NOT NULL,  -- Gross amount before taxes
    FED_TAX_WH      DECIMAL(10,2) NULL,
    STE_TAX_WH      DECIMAL(10,2) NULL,
    NET_AMT         DECIMAL(12,2) NOT NULL,  -- Net amount paid
    ...
    COLA_AMT        DECIMAL(10,2) NULL,      -- COLA component included in GROSS_AMT
...
CREATE TABLE PRISM_BENEFICIARY (
    ...
    SHARE_PCT       DECIMAL(5,2) NULL,       -- Percentage share (should sum to 100 per member per type)
    EFF_DT          DATE         NULL,
...
CREATE TABLE PRISM_QDRO (
    ...
    SHARE_AMT       DECIMAL(10,4) NULL,      -- Percentage (0-100) or dollar amount depending on SHARE_TYP
...
CREATE TABLE PRISM_DISABILITY (
    ...
    DISB_BENEFIT    DECIMAL(12,2) NULL,      -- Monthly disability benefit amount
    DISB_PCT        DECIMAL(5,2) NULL,       -- Disability percentage (if PART type)
...
CREATE TABLE PRISM_PLAN_PARAMS (
    ...
    PLAN_CD         VARCHAR(6)   NOT NULL,   -- References PRISM_MISC_CODES TYPE_CD='PLAN'. No FK.
    EFF_DT          DATE         NOT NULL,
    END_DT          DATE         NULL,       -- NULL = current. But see note about gaps.
    BENEFIT_MULT    DECIMAL(6,4) NULL,       -- Benefit multiplier (e.g. 0.0200 = 2.0%)
    NRM_RET_AGE     DECIMAL(4,1) NULL,       -- Normal retirement age (e.g. 65.0)
    ERY_RET_AGE     DECIMAL(4,1) NULL,       -- Earliest early retirement age (e.g. 55.0)
    ERY_PENALTY_RT  DECIMAL(6,4) NULL,       -- Early retirement reduction per year (e.g. 0.06 = 6%)
    ERY_MAX_PEN     DECIMAL(6,4) NULL,       -- Maximum early retirement penalty (e.g. 0.30 = 30%)
    FAS_PERIOD_MO   SMALLINT     NULL,       -- FAS averaging period in months (36 or 60)
    MAX_BENEFIT_PCT DECIMAL(5,4) NULL,       -- Max benefit as % of FAS (e.g. 0.75 = 75%)
    MIN_BENEFIT_AMT DECIMAL(10,2) NULL,      -- Minimum monthly benefit floor ($)
    VESTING_YRS     DECIMAL(4,1) NULL,       -- Years to full vesting
    CONTRIB_RATE_EE DECIMAL(6,4) NULL,       -- Employee contribution rate
    CONTRIB_RATE_ER DECIMAL(6,4) NULL,       -- Employer contribution rate
    COLA_CAP_PCT    DECIMAL(5,4) NULL,       -- COLA cap (e.g. 0.03 = 3%)
...
CREATE TABLE PRISM_EMPR_LIST (
    ...
    LEAVE_DT        DATE         NULL,
    CONTRIB_RATE    DECIMAL(6,4) NULL,       -- Employer contribution rate (may differ from plan default)
...
CREATE TABLE PRISM_COLA_HIST (
    ...
    PRE_COLA_AMT    DECIMAL(12,2) NOT NULL,  -- Monthly amount before COLA
    COLA_RATE_APPL  DECIMAL(6,4) NULL,       -- Rate actually applied (may be less than CPI due to cap)
    CPI_RATE        DECIMAL(6,4) NULL,       -- CPI rate for the period
    BANKED_RATE     DECIMAL(6,4) NULL,       -- Rate banked for future (if CPI > cap)
    POST_COLA_AMT   DECIMAL(12,2) NOT NULL,  -- Monthly amount after COLA
