...
import type { BenefitCalcResult, EligibilityResult, ScenarioResult } from '@/types/BenefitCalculation';
...
export function useEligibility(memberID: number, retirementDate?: string) {
  return useQuery<EligibilityResult>({
    ...
    queryFn: () => intelligenceAPI.evaluateEligibility(memberID, retirementDate) as Promise<EligibilityResult>,
    enabled: memberID > 0,
  });
}

export function useBenefitCalculation(memberID: number, retirementDate: string) {
  return useQuery<BenefitCalcResult>({
    queryKey: ['benefit', memberID, retirementDate],
    queryFn: () => intelligenceAPI.calculateBenefit(memberID, retirementDate) as Promise<BenefitCalcResult>,
