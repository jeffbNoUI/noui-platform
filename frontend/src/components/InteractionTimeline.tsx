...
const channelConfig: Record<InteractionChannel, ChannelConfig> = {
  phone_inbound: { label: 'Phone', icon: <PhoneIcon />, dotColor: 'bg-blue-500', bgColor: 'bg-blue-50' },
  phone_outbound: { label: 'Phone', icon: <PhoneIcon />, dotColor: 'bg-blue-500', bgColor: 'bg-blue-50' },
  email_inbound: { label: 'Email', icon: <EmailIcon />, dotColor: 'bg-green-500', bgColor: 'bg-green-50' },
  email_outbound: { label: 'Email', icon: <EmailIcon />, dotColor: 'bg-green-500', bgColor: 'bg-green-50' },
  secure_message: { label: 'Message', icon: <MessageIcon />, dotColor: 'bg-purple-500', bgColor: 'bg-purple-50' },
  walk_in: { label: 'Walk-In', icon: <WalkInIcon />, dotColor: 'bg-amber-500', bgColor: 'bg-amber-50' },
  portal_activity: { label: 'Portal', icon: <SystemIcon />, dotColor: 'bg-cyan-500', bgColor: 'bg-cyan-50' },
  mail_inbound: { label: 'Mail', icon: <MailIcon />, dotColor: 'bg-teal-500', bgColor: 'bg-teal-50' },
  mail_outbound: { label: 'Mail', icon: <MailIcon />, dotColor: 'bg-teal-500', bgColor: 'bg-teal-50' },
  internal_handoff: { label: 'Handoff', icon: <SystemIcon />, dotColor: 'bg-gray-500', bgColor: 'bg-gray-50' },
  system_event: { label: 'System', icon: <SystemIcon />, dotColor: 'bg-gray-400', bgColor: 'bg-gray-50' },
...
function TimelineEntryDetail({ interactionId }: { interactionId: string }) {
  ...
  if (isLoading) {
    return (
      <div className="mt-2 rounded-md bg-gray-50 p-3 text-xs text-gray-400">
        Loading details...
      </div>
    );
  }
      ...
      {notes.map((note) => (
        <div key={note.noteId} className="rounded-md border border-gray-100 bg-gray-50 p-3">
          <div className="flex items-center gap-2 text-xs flex-wrap">
            ...
            {note.outcome && (
              <span className="rounded bg-blue-100 px-1.5 py-0.5 text-blue-700">{note.outcome.replace(/_/g, ' ')}</span>
            )}
            {note.sentiment && note.sentiment !== 'neutral' && (
              <span className={`rounded px-1.5 py-0.5 font-medium ${
                ...
                note.sentiment === 'escalation_risk' ? 'bg-orange-100 text-orange-700' :
            ...
            {note.aiSuggested && (
              <span className="rounded bg-violet-100 px-1.5 py-0.5 text-violet-700">AI Suggested</span>
            )}
          </div>
      ...
      {commitments.map((c) => (
        <div key={c.commitmentId} className="rounded-md border border-blue-100 bg-blue-50 p-3">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className={`rounded px-1.5 py-0.5 font-medium ${commitmentStatusColor(c.status)}`}>
              {c.status}
            </span>
          </div>
          <p className="mt-1 text-sm text-gray-800">{c.description}</p>
          <p className="mt-1 text-xs text-gray-500">
            Due: {new Date(c.targetDate + 'T00:00:00').toLocaleDateString()} &middot; Owner: {c.ownerAgent}
          </p>
        </div>
      ))}
    </div>
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {entry.visibility === 'internal' && (
              <span className="rounded bg-gray-200 px-1.5 py-0.5 text-xs text-gray-500">Internal</span>
            )}
            {entry.hasNotes && (
              <span className="rounded bg-amber-100 px-1.5 py-0.5 text-xs font-medium text-amber-700">[notes]</span>
            )}
          </div>
        ...
        <div className="mt-1.5 flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            {entry.agentId && (
              <span className="text-xs text-gray-500">Agent: {entry.agentId}</span>
            )}
            {entry.summary && (
              <p className="mt-0.5 text-sm text-gray-700">{entry.summary}</p>
            )}
          </div>
