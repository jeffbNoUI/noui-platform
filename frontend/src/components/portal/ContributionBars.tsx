...
export default function ContributionBars({ data, width = 280, height = 140 }: Props) {
  const pL = 4, pR = 4, pB = 20, pT = 4;
  const w = width - pL - pR, h = height - pT - pB;
  const max = Math.max(...data.map(d => d.employee + d.employer));
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${width} ${height}`}>
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            <rect x={x} y={pT + h - totalH} width={barW} height={totalH - empH} rx="4" ry="4" fill={C.sage} opacity="0.7" />
            <rect x={x} y={pT + h - empH} width={barW} height={empH} rx="0" fill={C.gold} opacity="0.7" />
            <text x={x + barW / 2} y={height - 4} textAnchor="middle" fontSize="9" fill={C.textTertiary} fontFamily={BODY}>{d.year}</text>
          </g>
        );
      })}
    </svg>
