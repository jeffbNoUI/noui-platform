...
export default function BenefitProjectionChart({ data, width = 540, height = 220 }: Props) {
  ...
  const max = Math.max(...data.map(d => d.projected)) * 1.12;
  ...
  data.forEach((d, i) => {
    ...
    if (dist < minDist) { minDist = dist; todayIdx = i; }
  });
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${width} ${height}`} style={{ overflow: 'visible' }}>
      ...
      {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={pL - 10} y={y + 3} textAnchor="end" fontSize="10" fill={C.textTertiary} fontFamily={MONO}>
      ...
      />
      <polyline points={toPath(contributed, false)} fill="none" stroke={C.gold} strokeWidth="2" strokeDasharray="6 3" opacity="0.6" />

      {/* Projected area */}
      <path d={`${toPath(projected)} L ${pL + w},${pT + h} L ${pL},${pT + h} Z`} fill="url(#projGrad)" />
      <path d={toPath(projected)} fill="none" stroke={C.sage} strokeWidth="2.5" strokeLinecap="round" />

      {/* Conservative line */}
      <path d={toPath(conservative)} fill="none" stroke={C.textTertiary} strokeWidth="1.5" strokeDasharray="4 4" opacity="0.5" />

      {/* Year labels */}
      {data.map((d, i) => i % 2 === 0 && (
        <text key={i} x={pL + (i / (data.length - 1)) * w} y={pT + h + 20} textAnchor="middle" fontSize="10" fill={C.textTertiary} fontFamily={BODY}>
          {d.year}
        </text>
      ))}

      {/* Current marker */}
      <line x1={todayX} y1={pT} x2={todayX} y2={pT + h} stroke={C.sage} strokeWidth="1" strokeDasharray="3 3" opacity="0.4" />
      <circle cx={todayX} cy={todayY} r={5} fill={C.cardBg} stroke={C.sage} strokeWidth="2.5" />
      <text x={todayX} y={pT - 4} textAnchor="middle" fontSize="10" fontWeight="600" fill={C.sage} fontFamily={BODY}>Today</text>
    </svg>
  );
}
