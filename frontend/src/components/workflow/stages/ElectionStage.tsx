...
function buildPaymentOptions(calculation?: BenefitCalcResult): PaymentOption[] {
...
export default function ElectionStage({
  ...
  return (
    <div>
            ...
            >
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div
                      className={`text-sm font-medium ${isSelected ? 'text-iw-sage' : 'text-gray-700'}`}
                    >
                      {opt.label}
                    </div>
                ...
                <span
                  className={`font-mono font-bold text-sm ${isSelected ? 'text-iw-sage' : 'text-gray-700'}`}
                >
      ...
      {isMarried &&
        (selectedOption === 'j100' || selectedOption === 'j75' || selectedOption === 'j50') && (
          <Callout
            type="info"
      ...
      >
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-5 rounded-full relative transition-colors ${iprEnrolled ? 'bg-iw-sage' : 'bg-gray-300'}`}
            >
              <div
                className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${iprEnrolled ? 'left-[22px]' : 'left-0.5'}`}
              />
            </div>
          ...
          <div className="text-right">
            <div className="text-xs text-gray-500">
              ...
              <span className="font-mono font-semibold">
                {fmt(ipr?.non_medicare_monthly || ipr?.monthly_amount)}
              </span>
              /mo
            </div>
            <div className="text-xs text-gray-500">
              Post-Medicare:{' '}
              <span className="font-mono font-semibold">{fmt(ipr?.medicare_monthly)}</span>/mo
            </div>
          </div>
        </div>
      </div>
