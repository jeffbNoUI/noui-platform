    ...
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <span
            className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium text-white ${tierColors[member.tier_code] || 'bg-gray-500'}`}
          >
            {tierLabel(member.tier_code)}
          </span>
          <span
            className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium text-white ${statusColors[member.status_code] || 'bg-gray-500'}`}
          >
      ...
      <div className="mt-3 grid grid-cols-5 gap-4 border-t border-gray-100 pt-3 text-sm">
        <div>
      ...
      {member.term_date && (
        <div className="mt-2 text-sm">
          ...
          <span className="font-medium text-red-600">{formatDate(member.term_date)}</span>
        </div>
      )}
    </div>
  );
